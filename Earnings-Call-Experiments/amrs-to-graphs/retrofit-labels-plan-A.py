"""
BONUS: nambahin label + jumlah relasi PERSIS ke file .pt PLAN A yang SUDAH
...
amrs-to-graphs-plan-A-hk-finbert-new.py (bagian pembentukan graf, minus
pemanggilan BERT): hanya doc_cls (index 0) + node ::node per kalimat,
dan tiap kalimat baru doc_cls disambungkan ke ::root kalimat SEBELUMNYA
(tanpa snt_cls, tanpa rantai root-ke-root). JANGAN diubah agar jumlah
relasi yang dihasilkan tetap cocok dengan file .pt yang sudah ada.
...
4. Di akhir, CSV graph_stats_plan_A.csv ikut dibuat/diupdate (filename,
...
SRC_DIR = "/kaggle/input/datasets/amzar11/graf-plan-a/truly-all-results-graphs-hk-finbert-plan-A"

# Tulis hasil yang sudah dikasih label ke sini (WAJIB writable, di /kaggle/working):
DST_DIR = 'truly-all-results-graphs-hk-finbert-plan-A'
...
def parse_amr_labels_and_relations(fname):
    ...
    graph di amrs-to-graphs-plan-A-hk-finbert-new.py (edge doc_cls <->
    ::root kalimat sebelumnya, TANPA snt_cls, TANPA rantai root-ke-root),
    tapi tanpa hitung embedding sama sekali. Return (words, entities,
    num_relations). Urutan words/entities sejajar dengan urutan node_index
    waktu graph dibuat aslinya (index 0 = doc_cls).
    """
...
csv_path = 'graph_stats_plan_A.csv'
