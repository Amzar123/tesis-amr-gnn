"""
BONUS: nambahin label + jumlah relasi PERSIS ke file .pt PLAN D yang SUDAH
...
amrs-to-graphs-plan-D-hk-finbert-new-amd.py (bagian pembentukan graf, minus
pemanggilan BERT): sama seperti Plan C (tiap kalimat dapat node "snt_cls"
+ rantai snt_cls(sblm) <-> snt_cls(skrg)), TAPI doc_cls (index 0) JUGA
disambungkan langsung ke SETIAP node individual di kalimat sebelumnya
(bukan cuma lewat snt_cls) -- inilah pembeda Plan D dari Plan C. Plan D
juga TIDAK memakai baris ::root. JANGAN diubah agar jumlah relasi & jumlah
node yang dihasilkan tetap cocok dengan file .pt yang sudah ada.
...
4. Di akhir, CSV graph_stats_plan_D.csv ikut dibuat/diupdate (filename,
...
SRC_DIR = "/kaggle/input/datasets/amzar11/graf-plan-d/truly-all-results-graphs-hk-finbert-plan-D"

# Tulis hasil yang sudah dikasih label ke sini (WAJIB writable, di /kaggle/working):
DST_DIR = 'truly-all-results-graphs-hk-finbert-plan-D'
...
def parse_amr_labels_and_relations(fname):
    ...
    graph di amrs-to-graphs-plan-D-hk-finbert-new-amd.py: tiap kalimat
    dikasih 1 node "snt_cls" ekstra yang disambungkan ke semua node di
    kalimat SEBELUMNYA + ke doc_cls, plus rantai snt_cls(sblm) <->
    snt_cls(skrg); DAN doc_cls disambungkan langsung ke SETIAP node
    individual di kalimat sebelumnya juga. Tanpa hitung embedding sama
    sekali. Return (words, entities, num_relations). Urutan words/entities
    sejajar dengan urutan node_index waktu graph dibuat aslinya (index 0 =
    doc_cls).
    """
    ...
    while True:
        ...
        if line[:7] == "# ::tok":
            ...
            # PLUS edge doc_cls <-> tiap node itu juga,
            # baru edge doc_cls <-> snt_cls KALIMAT SEBELUMNYA
...
csv_path = 'graph_stats_plan_D.csv'
