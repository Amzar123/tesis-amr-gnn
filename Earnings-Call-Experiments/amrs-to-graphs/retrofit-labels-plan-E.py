"""
BONUS: nambahin label + jumlah relasi PERSIS ke file .pt PLAN E yang SUDAH
...
amrs-to-graphs-plan-E-hk-finbert-new-amd.py (bagian pembentukan graf, minus
pemanggilan BERT): TIDAK ada snt_cls sama sekali (cuma doc_cls di index 0),
dan doc_cls disambungkan langsung ke SETIAP node individual di kalimat
SEBELUMNYA (bukan cuma ke root-nya). Plan E juga TIDAK memakai baris
::root. JANGAN diubah agar jumlah relasi yang dihasilkan tetap cocok
dengan file .pt yang sudah ada.
...
4. Di akhir, CSV graph_stats_plan_E.csv ikut dibuat/diupdate (filename,
...
SRC_DIR = "/kaggle/input/datasets/amzar11/graf-plan-e/truly-all-results-graphs-hk-finbert-plan-E"

# Tulis hasil yang sudah dikasih label ke sini (WAJIB writable, di /kaggle/working):
DST_DIR = 'truly-all-results-graphs-hk-finbert-plan-E'
...
def parse_amr_labels_and_relations(fname):
    ...
    graph di amrs-to-graphs-plan-E-hk-finbert-new-amd.py: TANPA snt_cls,
    doc_cls disambungkan langsung ke SETIAP node di kalimat SEBELUMNYA.
    Tanpa hitung embedding sama sekali. Return (words, entities,
    ...
    while True:
        line = flines[count]
        if line[:7] == "# ::tok":
            # edge doc_cls <-> tiap node di kalimat SEBELUMNYA
...
csv_path = 'graph_stats_plan_E.csv'
