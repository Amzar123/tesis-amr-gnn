"""
BONUS: nambahin label + jumlah relasi PERSIS ke file .pt PLAN B yang SUDAH
...
amrs-to-graphs-plan-B-hk-finbert-new-amd.py (bagian pembentukan graf, minus
pemanggilan BERT): doc_cls (index 0) + node ::node per kalimat, tiap
kalimat baru doc_cls disambungkan ke ::root kalimat SEBELUMNYA, DAN ada
rantai tambahan root(kalimat sebelumnya) <-> root(kalimat sekarang) --
inilah pembeda Plan B dari Plan A. JANGAN diubah agar jumlah relasi yang
...
4. Di akhir, CSV graph_stats_plan_B.csv ikut dibuat/diupdate (filename,
...
SRC_DIR = "/kaggle/input/datasets/amzar11/graf-plan-b/truly-all-results-graphs-hk-finbert-plan-B"

# Tulis hasil yang sudah dikasih label ke sini (WAJIB writable, di /kaggle/working):
DST_DIR = 'truly-all-results-graphs-hk-finbert-plan-B'
...
def parse_amr_labels_and_relations(fname):
    ...
    graph di amrs-to-graphs-plan-B-hk-finbert-new-amd.py (edge doc_cls <->
    ::root kalimat sebelumnya, PLUS rantai root(sblm) <-> root(skrg), TANPA
    snt_cls), tapi tanpa hitung embedding sama sekali. Return (words,
    ...
    current_snt = ''
    current_snt_root_doc_idx = None
    previous_snt_root_doc_idx = None
    id_to_doc_idx = {}
    count = 0
    ...
    while True:
        line = flines[count]
        if line[:7] == "# ::tok":
            # edge doc_cls <-> root kalimat SEBELUMNYA
            if current_snt_root_doc_idx is not None:
                edge_list1.append(0)
                edge_list2.append(current_snt_root_doc_idx)
            previous_snt_root_doc_idx = current_snt_root_doc_idx
            id_to_doc_idx = {}
            ...
            id_to_doc_idx[snt_node_id] = node_index
            node_index += 1
            count += 1
            continue
        elif line[:8] == "# ::edge":
            ...
            n1 = id_to_doc_idx[key1]
            n2 = id_to_doc_idx[key2]
            ...
            current_snt_root_doc_idx = id_to_doc_idx[snt_root_id]
            # rantai: root kalimat SEBELUMNYA <-> root kalimat SEKARANG
            if previous_snt_root_doc_idx is not None:
                edge_list1.append(previous_snt_root_doc_idx)
                edge_list2.append(current_snt_root_doc_idx)
        count += 1
        if count == len(flines):
            edge_list1.append(0)
...
csv_path = 'graph_stats_plan_B.csv'
